...
```bash
webreader$ sam build --use-container
```
...
```bash
webreader$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
webreader$ sam local start-api
webreader$ curl http://localhost:3000/
```
...
```bash
webreader$ sam logs -n HelloWorldFunction --stack-name webreader --tail
```
...
```bash
webreader$ pip install -r tests/requirements.txt --user
# unit test
webreader$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
webreader$ AWS_SAM_STACK_NAME="webreader" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "webreader"
```
